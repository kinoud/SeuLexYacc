...
- Lex对词素属性值的处理。在我们的成品中，与词素属性值相关的变量是`yylval`，但是它仅仅可以是一个整型值。我们并未在词素的属性值上做进一步的探讨，但是这种探讨是有必要的，比如`3.14`是一个`NUMBER`类型的词素，它的属性值就是一个`double`或`float`类型的值，记录属性值对后续的编译过程是必要的。
- Yacc中的冲突检测报告。我们虽然可以对冲突行为进行自定义的或默认的选择，但是并未生成报告信息，这对于使用者来说是不友好的。
- 一些细节上的处理并不优雅。例如我们规定-2代表增广文法开始符号的Token ID，-1代表一切被忽略（无动作）的终结符的Token ID，0表示输入终止符，这些人为规定的规则会增加用户的负担。还有，我们规定用户定义的Token ID的范围是`[256,499]`，尽管这个范围是可以修改的，但是仍然会给用户带来不友好的体验。
- 语法树节点的属性值。目前，我们的成品中语法树中的每个节点可以有且仅可以有1个整型的属性值，可以通过产生式归约动作定义它们的产生方法。然而实际上，一个节点应当拥有一个更加通用的属性集，例如我们可以给每个节点分配一个属性结构体。对于此我们并没有进行更深入的探讨。